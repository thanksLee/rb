<?php
function getAgoGrade($keyword,$d1,$d2)
{
	global $SR,$date,$table,$DB_CONNECT,$s;
	$d_regis1	= date('Ymd',mktime(0,0,0,substr($date['today'],4,2),substr($date['today'],6,2)-$d2,substr($date['today'],0,4)));
	$d_regis2	= date('Ymd',mktime(0,0,0,substr($date['today'],4,2),substr($date['today'],6,2)-$d1,substr($date['today'],0,4)));
	$WHEREIS = 'where (d_regis between '.$d_regis1.' and '.$d_regis2.") and keyword='".$keyword."' and site=".$s;
...
function getGradeArr($d1,$d2)
{
	...
	$d_regis1	= date('Ymd',mktime(0,0,0,substr($date['today'],4,2),substr($date['today'],6,2)-$d2,substr($date['today'],0,4)));
	$d_regis2	= date('Ymd',mktime(0,0,0,substr($date['today'],4,2),substr($date['today'],6,2)-$d1,substr($date['today'],0,4)));
	$WHEREIS = 'where (d_regis between '.$d_regis1.' and '.$d_regis2.') and site='.$s;
