...
$_albums = explode(',',$d['mediaset']['category2']);
?>
...
<div id="catebody">
	...
	<div id="category">
		<div class="title">
			<span class="add">
			<a href="#." title="앨범 추가" onclick="AddAlbumLayer('category2',event);"><img src="<?php echo $g['img_core']?>/_public/btn_add.gif" alt="" /></a>
			<input type="image" src="<?php echo $g['img_core']?>/_public/btn_save.gif" title="순서저장" />
			</span>
			동영상 앨범
		</div>
		<?php if($d['mediaset']['category2']):?>
		<div class="tgap"></div>
		<ul id="menuorder">
		...
		<tr class="layout1">
		<td class="ltd">
			<a href="#." onclick="crLayer('동영상등록','<?php echo $g['s']?>/?r=<?php echo $r?>&m=<?php echo $module?>&mod=vod&album=<?php echo urlencode($_ab)?>','iframe',728,580,'15%');"><img src="<?php echo $g['img_core']?>/_public/ico_folder_0<?php echo $_num?2:1?>.gif" alt="동영상등록" /></a>
		...
		<td class="rtd">
			<a href="<?php echo $g['s']?>/?r=<?php echo $r?>&amp;m=<?php echo $module?>&a=category_delete&category=category2&amp;name=<?php echo urlencode($_ab)?>" onclick="return hrefCheck(this,true,'앨범내의 파일까지 모두 삭제됩니다.\n정말로 삭제하시겠습니까?');"><img src="<?php echo $g['img_core']?>/_public/btn_del_s01.gif" alt="삭제" title="삭제" /></a>
		</td>
		</tr>
		</table>
		</li>
	...
	<div id="catinfo">
		<div class="title">
			...
			<div class="xright">
				<?php if($album):?><a href="#." onclick="crLayer('동영상등록','<?php echo $g['s']?>/?r=<?php echo $r?>&m=<?php echo $module?>&mod=vod&album=<?php echo urlencode($album)?>','iframe',728,580,'15%');">동영상등록</a><?php endif?>
			</div>
			<div class="clear"></div>
		</div>
		...
		<div class="photobox">
			...
			<div class="photo">
				<div class="pic" style="background:url('<?php echo $_R['url'].$_R['category'].'/thumb2_'.$_R['name']?>') center center no-repeat;" onmouseover="imgShow('<?php echo $_R['url'].$_R['category'].'/thumb1_'.$_R['name']?>',event);" onmouseout="imgHide();"><input type="checkbox" name="photo_members[]" value="<?php echo $_R['uid']?>" /></div>
				...
				<div class="info">(<?php echo number_format($_R['width'])?>*<?php echo number_format($_R['height'])?> / <?php echo getSizeFormat($_R['size'],1)?> / <?php echo strtoupper($_R['ext'])?>)</div>
			</div>
			<?php endwhile?>
		...
		<?php if(!$NUM):?>
		<div class="none">등록된 동영상이 없습니다.</div>
		<?php endif?>	
		<div class="pagebox01">
...
function AddAlbumLayer(fd,e)
{
	...
	html += '<div style="border-bottom:#dfdfdf solid 1px;padding:0 0 7px 2px;color:#666666;"><b>동영상앨범 만들기</b><img src="<?php echo $g['img_module_admin']?>/btn_close.gif" alt="닫기" style="position:relative;left:185px;top:2px;cursor:pointer;" onclick="AddAlbumLayerClose();" /></div>';
